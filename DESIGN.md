...
- **Slate Teal Deep** (`primary-strong`): hover state of the primary button; text colour of the selected template name, the primary chip, a hovered FAQ question, a touched reference numeral, and the active topic's numeral in the FAQ index.
...
- **Panel** (`panel`): one step down; the document-preview frame, disabled buttons, hovered segments and quiet buttons, the lifecycle stage tile, and the six step cards under §7 (the page's only filled cards).
...
- **Line Strong** (`line-strong`): emphasised hairline; table heads, chip and quiet-button borders, the segmented-cell frame, dashed placeholders, lifecycle arrows, unread chart bars, the FAQ index's top rule, the closed clause's 2px top rule, the reference leader at rest, the FAQ close's top rule.
...
- **Label** (400, 0.75rem, `muted`): captions, legends, table heads (500), tree label, FAQ facts, the index numeral. Hints, permalinks, index counts, and the index head (uppercase, 0.04em) go to 0.6875rem.
...
- **Fine** (mono, 0.625rem, 0.08em, `muted`): the confidentiality stamp and fine print inside the simulated document. Reserved for text that is inside an instrument, never for page copy.
...
**The Inside-the-Instrument Rule.** `fine` (0.625rem) appears only inside a rendered instrument (the document sheet). Page copy, captions, and hints never go below 0.6875rem.
...
**The hero** is the one section with its own grid: a headline band (`minmax(0,1fr) auto`, bottom-aligned, 2rem × 3rem gaps) over a deck split `minmax(max-content, 1.3fr) | minmax(19rem, 1fr)` with a 2.5rem gutter (the matrix column is never narrower than the table, so the two never collide between 1024 and 1130px; the room header is `contain: inline-size` so its single-line width does not count); matrix left, document preview over the activity list right (`grid-template-rows: auto 1fr`), and a third row `rec rec` spanning both columns 2.5rem below for the app recording (`DemoVideo`: hairline top rule, a head like the activity list's with the title at 0.8125rem 600 `ink` and the recording's facts at 0.6875rem `muted` on the right, then the cropped screen recording at its native 1044 × 568 in a 1px `line` 8px frame; it autoplays muted only while 40% on screen, never under `prefers-reduced-motion`, and keeps native controls so it can be paused). Its reference numeral `§1` sits at the head of the room name above the matrix. **Under 1024px the deck is a block and the preview aside is a bottom sheet:** `position: sticky; bottom: 0` on `ground` at 92% over an 8px blur with a `line-strong` top rule, full-bleed to the container edges, so a tap on the matrix shows its consequence on the same screen. While the sheet floats (a sentinel after its natural position, observed with `IntersectionObserver`, sets `.stuck`) it carries the newest activity lines, three beside the preview on tablets and one clamped line under it on phones; once it settles, the strip hides and the full activity list follows directly below. The document sheet inside is compact there (11rem, 7.5rem on phones, 6.5rem under 700px tall; facts table and fine print hidden, excerpt clamped to two lines, download hint hidden on phones). Matrix cells carry `scroll-margin-block: 4.5rem 26rem` so a focused cell scrolls clear of the nav and the sheet. Viewports under 560px tall (a phone in landscape) get no sheet and flow instead. The hint under the room name names where the preview is per layout ("di kanan" / "di bawah").
...
- Q&A: three equal silos divided by vertical hairlines under a `line-strong` rule; the owner silo lifts to `surface` with an ink top edge and is the reading path, so the same question in a guest silo is set one step lighter (`ink-3`, answers `muted`) and reads as what that side sees.
- Engagement: the same `DemoVideo` block as the hero's third row, alone in the content column: the app's pages-read report as a screen recording (1034 × 552), no simulated card beside it.
...
- Security: the closing argument, so it is the one sheet with more air (7rem vertical padding, 4.5rem on phones) and the page's one larger lede (1.125rem). A two-column ledger: the "do" column a definition grid (`12rem | 1fr`, stacked under 1200px) with terms at 1.0625rem 600 `ink` and definitions at 0.9375rem in a 52ch measure; the "don't" column single-column, each term preceded by a mono uppercase `muted` label "tidak diklaim" (0.6875rem, 0.08em) instead of a strike-through, so the term stays legible and the mark carries the meaning at full contrast.
- Setup: six step cards in a two-column grid under a `line-strong` rule (one column under 768px); each card is a three-track subgrid (title, body, artifact) so the rule above every artifact sits on one line across the row.
...
- **Primary:** slate teal on primary content, 500 weight, 2.75rem min height, 0.625rem × 1.125rem padding, 0.9375rem type. The hero, close, and FAQ close CTAs grow to 3rem and 1rem type (`TrialCta` size `lg`, full width under 768px).
- **The trial action has three honest states** (`TrialCta.svelte`, from `app-url.ts`): the app's sign-up link when `PUBLIC_APP_URL` exists (sign-in links render only then); a mailto "Beritahu saya saat uji coba dibuka" (nav: "Kabari saya") when only `PUBLIC_CONTACT_EMAIL` exists; and a plain 500-weight `ink` statement "Uji coba 30 hari dibuka segera." when neither does, with nothing in the nav. It is never a link to nowhere and there is no developer note in visitor view. On the landing page the nav's copy of the action is `btn-quiet` while the hero's own button is on screen (an `IntersectionObserver` under the nav) and turns `primary` once it scrolls away, so a viewport never carries two teal actions; on the FAQ it is primary throughout.
...
### Cards / Containers
- **Corner Style:** 8px.
- **Background:** `surface` for the engagement card and lifecycle room cards; `panel` for the document-preview frame, which then holds a `surface` page inside it, and for the §7 step cards, which hold `surface` controls.
...
The signature. A real table: `line-strong` under the head, `line` under every row, 0.8125rem, folder names in `ink` at 450 weight, group names in `muted` 0.75rem. Each cell is a segmented four-toggle control (view, watermark, download, original). The state lives in the URL; the focused cell is ringed in teal and drives the preview beside it. **The grid is one Tab stop** (roving `tabindex`): arrows move between segments, across groups on the same row, and up and down folders; Home and End reach the row's first and last segment, PageUp and PageDown the first and last folder; Space toggles. Keyboard movement moves the ring and the preview exactly as a pointer does, and below 640px it switches the group tab. Every segment carries a `title` that names its consequence in the app's own words ("Unduh PDF tanpa watermark: rendisi bersih. Menyalakan Unduh, mematikan Watermark."); the four glyphs are an eye, a hatched document, an arrow into a tray, and a document with an arrow leaving it, so the two downloads never read alike. Back, forward, and reset append their own activity line ("kembali ke pengaturan sebelumnya", "mengembalikan pengaturan awal"), so the trail never contradicts the grid; hash jumps that leave the grid alone add nothing. The table is described by the rule text (`aria-describedby`), which carries a visually hidden keyboard hint. **One live region:** a visually hidden status line under the matrix announces each action as one sentence (the activity line it appended); the preview and the activity list are not live regions.
...
### The Activity Line
An ordered list under a `line` rule, 3rem mono time column plus text, 0.375rem row padding, hairline between rows. Newest first, newest darkest, capped at seven. Human actors are Inter 500; machine actors (an email) are mono 400 at 0.75rem. Not a live region; the hero's status line announces for it.

### The Step Card
The setup section's instrument (§7): an ordered list of six `panel` cards (the only filled cards on the page; controls and chips inside them stay `surface`, and the stage tile turns `surface` with a `line` border so it still reads), 1px `line` border, 8px, 1.25rem × 1.5rem padding (1rem on mobile). Head: a mono step numeral (0.8125rem 500, 0.02em, `muted`) before a 0.9375rem 500 `ink` title; body 0.875rem `ink-2`. Under a `line` rule at the foot sits the artifact: the state the app really shows at that step, drawn from the demonstration data. In order: the verification code as five mono digits and a teal caret in one segmented frame; the room row (stage tile, name, `Status: Persiapan` chip); the template row with its folder-count chip and the `chip-sans` apply control; two document rows with mono page count and version; one permission cell at rest (on is ink, off is muted) beside its group-and-folder label; the invitation line in mono with the `chip-sans` open control and the `chip-primary` active status. Artifacts are inert and decorative bits are `aria-hidden`; the body text carries the same facts.

### The Reference Numeral
The device that binds the landing page to the FAQ. `REFS` in `src/lib/refs.ts` numbers seven landing sections §1 to §7 in landing order; `secnum()` prints the numeral. On the landing page it is `.secnum`: inline at the head of the section H2 (or the hero room name), mono 0.8125rem 500, 0.02em, `muted`, raised 0.35em with 0.625rem after it, so the headline still reads as one line. In the FAQ margin the same `§n` is mono 0.8125rem in `ink`, beside a two-line Inter label (a 0.6875rem `muted` descriptor over the 0.8125rem `ink` section name), reached by a 1.75rem leader from the answer column's edge. Hover or focus pulls the leader taut (2px, `primary`), fills the ring, and turns the numeral `primary-strong` with the label underlined. Under 768px the leader is a static ring inline before the numeral.
...
- **Don't** set page copy, captions, or hints below 0.6875rem; `fine` lives only inside the document sheet.
