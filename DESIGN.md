...
- Engagement: a `surface` chart card: a three-column facts grid (reader, document, total, longest, pages read; two columns on phones) over a 24-bar grid at 9rem height, unread pages drawn as no bar at all, and a 60ch legend that only explains the bars.
...
Twenty-four bars in `primary` (unread pages in `line-strong`) at 9rem, drawn with `aria-hidden` for sighted readers; the data itself is a visually hidden table (page, dwell) with the read-count caption, so nothing in the chart is focusable. Hover shows a mono tooltip; the longest-read bar's value is always printed.
